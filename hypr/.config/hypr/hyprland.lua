...
local function get_hostname()
    ...
    -- Clean up trailing newlines or whitespace characters
    return result:gsub("%s+", "")
end

-- Get current hostname (e.g., "desktop-pc" or "thinkpad")
local hostname = get_hostname()

-- Construct the absolute path inside your machine-specific folder
local host_config_path = os.getenv("HOME") .. "/.config/hypr/machine-specific/" .. hostname .. ".lua"

-- Attempt to parse and execute the specific file
local load_chunk, err = loadfile(host_config_path)

-- Define monitor placeholder variable
local monitors = {}

if load_chunk then
    -- Execute file and grab returned left and right monitor variables
    monitors = load_chunk()
else
    -- Fallback handling if a machine file does not exist for this host
    print("No custom profile found at: " .. host_config_path .. ". Error: " .. tostring(err))
    
    -- Set safe defaults so your main script doesn't crash
    monitors = { left = "HDMI-A-2", right = "HDMI-A-1" }
end
...
--Source: ~/.config/hypr-static/machine.lua — convert this file to Lua and ensure it is on Lua's package.path.
--dofile(os.getenv("HOME") .. "/.config/hypr-static/machine.lua")
...
local menu = "fuzzel"
local browser = "~/.config/hypr/scripts/firefoxprofile.sh"
...
--hl.env("QT_TPA_PLATFORMTHEME", "qt6ct")
...
hl.bind(mainMod .. " + M", hl.dsp.exec_cmd("~/.config/hypr/scripts/power_menu.sh"))
...
hl.config({
    ...
    -- Requires playerctl
    -- Move Workspace to Monitor $MOD + CTRL + SHIFT + Direction
    --Resize Window
    ...
    --Bluetooth Manager Window Rules
    --HyprLauncher Float, Focus
    --PavuControl
    --Audible Player Windows - Firefox
    --########################
    --	Workspaces	#
    --########################
    --#####################################
