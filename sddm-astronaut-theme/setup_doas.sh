...
install_gum() {
    ...
    case $mgr in
        pacman) doas pacman -S gum ;;
        dnf) doas dnf install -y gum ;;
        zypper) doas zypper install -y gum ;;
        xbps-install) doas xbps-install -y gum ;;
        # refrence https://github.com/basecamp/omakub/issues/222
        apt)
            doas mkdir -p /etc/apt/keyrings
            curl -fsSL https://repo.charm.sh/apt/gpg.key | doas gpg --dearmor -o /etc/apt/keyrings/charm.gpg
            echo "deb [signed-by=/etc/apt/keyrings/charm.gpg] https://repo.charm.sh/apt/ * *" | doas tee /etc/apt/sources.list.d/charm.list
            doas apt update && doas apt install -y gum ;;
...
install_deps() {
    ...
    case $mgr in
        pacman) doas pacman --needed -S sddm qt6-svg qt6-virtualkeyboard qt6-multimedia-ffmpeg ;;
        xbps-install) doas xbps-install -y sddm qt6-svg qt6-virtualkeyboard qt6-multimedia ;;
        dnf) doas dnf install -y sddm qt6-qtsvg qt6-qtvirtualkeyboard qt6-qtmultimedia ;;
        zypper) doas zypper install -y sddm libQt6Svg6 qt6-virtualkeyboard qt6-multimedia ;;
        apt) doas apt update && doas apt install -y sddm qt6-svg-dev qml6-module-qtquick-virtualkeyboard qt6-multimedia-dev ;;
...
install_theme() {
    ...
    # Backup and copy
    [[ -d "$dst" ]] && doas mv "$dst" "${dst}_$DATE"
    doas mkdir -p "$dst"
    spin "Installing theme files..." doas cp -r "$src"/* "$dst"/

    # Install fonts
    [[ -d "$dst/Fonts" ]] && spin "Installing fonts..." doas cp -r "$dst/Fonts"/* /usr/share/fonts/

    # Configure SDDM
    echo "[Theme]
    Current=$THEME_NAME" | doas tee /etc/sddm.conf >/dev/null

    doas mkdir -p /etc/sddm.conf.d
    echo "[General]
    InputMethod=qtvirtualkeyboard" | doas tee /etc/sddm.conf.d/virtualkbd.conf >/dev/null
...
select_theme() {
    ...
    doas sed -i "s|^ConfigFile=.*|ConfigFile=Themes/${theme}.conf|" "$METADATA"
...
enable_sddm() {
    ...
    doas systemctl disable display-manager.service 2>/dev/null || true
